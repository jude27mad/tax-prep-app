...
# Ontario Health Premium (2025)
def health_premium_2025(taxable: float) -> float:
    t = taxable
    if t <= 20_000: return 0.0
    if t <= 36_000: return min(300.0, 0.06*(t-20_000))
    if t <= 48_000: return min(450.0, 300.0 + 0.06*(t-36_000))
    if t <= 72_000: return min(600.0, 450.0 + 0.25*(t-48_000))
    if t <= 200_000: return min(750.0, 600.0 + 0.25*(t-72_000))
    return min(900.0, 750.0 + 0.25*(t-200_000))

# Ontario surtax (2025) — applied to Ontario tax AFTER credits
def surtax_2025(ont_tax_after_credits: float) -> float:
    t = ont_tax_after_credits
    if t <= 5_710: return 0.0
    if t <= 7_307: return round(0.20*(t-5_710), 2)
